...
TextureCubemap*
TextureCubemap::create(ImageData *right, ImageData *left, ImageData *top, ImageData *bottom, ImageData *front, ImageData *back)
{
...
TextureCubemap*
TextureCubemap::createWithFiles(const char *right, const char *left, const char *top, const char *bottom, const char *front, const char *back)
{
    std::vector<ImageData *> imageDataSet;
    imageDataSet.reserve(6);
    // Cubemap texture is somehow upside-down, so the image needs to be flipped vertically.
